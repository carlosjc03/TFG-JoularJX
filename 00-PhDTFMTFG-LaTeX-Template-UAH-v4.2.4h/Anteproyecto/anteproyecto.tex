...
\item Comprender cómo mide JoularJX el consumo energético a nivel de
  ...
  fuentes de datos (la interfaz Intel RAPL expuesta por el subsistema
  \texttt{powercap} del núcleo Linux) y formatos de salida.

\item Instalar y configurar JoularJX en un entorno GNU/Linux,
  documentando el proceso paso a paso, incluida la verificación de la
  disponibilidad de los contadores RAPL y los permisos de lectura sobre
  \texttt{/sys/class/powercap}.
...
\item \textbf{Instalación y configuración del entorno.} Instalación de
  JoularJX (\textit{jar} del agente) sobre GNU/Linux, comprobación de que
  el procesador expone los contadores RAPL a través del subsistema
  \texttt{powercap}, ajuste de los permisos de lectura de los ficheros
  \texttt{energy\_uj} y configuración del fichero
  \texttt{config.properties}. Verificación del funcionamiento con un
  programa Java de prueba. Descarga de Jackson~\cite{jackson} y
  Gson~\cite{gson} desde Maven Central.
...
\item Instalación de JoularJX sobre GNU/Linux, comprobación del acceso
  a los contadores RAPL y verificación del entorno con un programa de
  prueba.
...
\begin{itemize}
\item Ordenador personal con sistema operativo GNU/Linux (Ubuntu) y
  procesador Intel o AMD (a partir de Ryzen) con soporte de RAPL.
